...
## 2026-06-18: Build an uncensored Claude Code on-device via aichat + llm-functions (machine: motog-nethunter) — project "Heretic CLI"
**Decision:** Give the Venice GLM 4.7 Flash Heretic model Claude-Code-style agency (run shell, read/write files, execute code, fetch URLs) by wrapping it in `aichat` (harness/tool-call loop) + `sigoden/llm-functions` (tools, built by `argc`), pointed at Venice's OpenAI-compatible API. Established as an ongoing project with a living doc at `projects/heretic-cli/CLAUDE.md`. Aliases: `agent` (full tools), `coder` (file edit), `dual` (tmux split of agent + shell), `aiq` (strip `<think>`).

**Why:** The user wants a self-owned, uncensored equivalent of Claude Code — information sovereignty on their own rooted device, no remote content filter gatekeeping what they can ask (legit/defensive use). Agency lives in the harness, not the model weights, so the model just needs a tool-call loop around it. Chose **aichat** over LangChain/Aider/OpenCode because it's already installed and used daily, is the only CLI that natively supports `llm-functions`, is OpenAI-compatible (points straight at Venice), and adds no Python-agent-framework overhead on a 3.6GB-RAM phone. Chose **llm-functions** because tools are plain auditable Bash/Py/JS scripts, trivial to extend on-device. Verified native tool calling works for this exact model with a raw curl `tools` payload before building — not assumed. Chose **curl+pandoc** for URL fetch over the keyless Jina tool because Jina routes every fetched URL through a third party (`r.jina.ai`); curl+pandoc is fully local (opsec). Known ceilings recorded in the project doc: web search needs an external key (Venice has no search grounding), and phone hardware (camera/GPS/sensors/SMS/internal BT/Wi-Fi-monitor) is capped by the chroot, not the harness.

## 2026-08-08: Quality standard split into principle (memory) vs. enforcement (procedure)

**Decision:** The "build best-in-class or don't build" standard is now two artifacts. **Memory** (`feedback_quality_standard.md`) holds only the principle: *"Finished work we build together — anything with a user besides me, or that I'll maintain — should compete with the strongest relevant solutions in its domain. Scope may be smaller, but execution quality is not lowered to compensate. Prototypes may fall below that bar but must not be treated as finished. Before completion, compare the result against the established standard, identify remaining gaps, and support the judgment with evidence rather than assertion."* Same text added to `shared/CLAUDE-behavior.md` as a `## Quality Standard` block plus a claim-binding line. **Enforcement** lives in the `project-teardown` / `project-revision` skills: constraint provenance, gap dispositions, concrete-evidence definitions.

**Why:** The old formulation was a slogan. Slogans get satisfied rhetorically — the model asserts "production-grade" and moves on — and its "or we don't build at all" clause was a consequence never once executed, which teaches that the surrounding rules are decorative. The replacement binds a procedure with observable outputs. The split exists because memory loads unconditionally into every session including trivial ones, while skills are invoked situationally — so the scope gate is enforced structurally rather than by prose definition.

**Rejected, with reasons worth keeping:** (1) A pre-build competitor-research step gated on "any build" — too broad, obviously excessive in small cases, which trains the rule as aspirational and bleeds into cases that matter. (2) Banning specific words ("production-ready", "best-in-class") — attacks vocabulary rather than the claim, trivially paraphrased around. Replaced with: do not represent work as finished or as meeting the standard until the required comparison has been performed and reported. That keys on a fact about the transcript, not a self-judgment about quality.

**Credit:** The two rejections and the scope-gate clause are AJ's; the enforcement rules are Claude's.

## 2026-08-08: Skill edits go to `~/.codex/skills/` first, never only to the Claude port

**Decision:** `~/.codex/skills/{project-teardown,project-revision}` is the sole source of truth. Any edit lands there first; `~/.claude/skills/` is a downstream port refreshed by rsync.

**Why:** Claude edited only the ported copies on 2026-08-08. The documented re-port procedure (`rsync -a --exclude '__pycache__' ~/.codex/skills/<name>/ ~/.claude/skills/<name>/`) would have silently destroyed them on the next source update, with no error and no diff to notice. Caught before any re-port; back-ported; source and port re-verified byte-identical, both bundles valid, 109 tests pass (38 teardown + 71 revision).

## 2026-08-08: `ai-toolshed` gains a `case-studies/` category

**Decision:** Third top-level category alongside `skills/`, `plugins/`, `tools/`. Houses projects that contain **no AI** but were only possible because of AI. Distinct admission gate, whose non-negotiable criterion is that every entry states **what the AI could not do** — an entry with no recorded failures is not credible and does not ship. `case-studies/` chosen over `showcase/` deliberately: "showcase" invites portfolio vanity, "case study" forces the writeup to carry the value.
